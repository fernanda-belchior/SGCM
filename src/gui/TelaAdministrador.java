...
public class TelaAdministrador extends JFrame {
	...
	public TelaAdministrador() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(TelaAdministrador.class.getResource("/imagens/HC.png")));
		...
		JLabel lblMenuCadastro = new JLabel(" MENU - CADASTRO - ADMINISTRADOR");
	...
	public void cadastrar()throws ExcecaoDadoInvalido, ExcecaoElementoJaExistente, ExcecaoElementoInexistente, ExcecaoRepositorioVazio {
		...
		Administrador administrador = new Administrador(nomeA,cpfA,telefoneA,senhaA);
		this.fachada.inserir(administrador);
		JOptionPane.showMessageDialog(this,"CADASTRADO COM SUCESSO!");
		limparCampos();
		
		try{
			textArea_2.append(this.fachada.imprimirAdministrador(cpfA));
		} catch (ExcecaoDadoInvalido | ExcecaoElementoInexistente e) {
			JOptionPane.showMessageDialog(null,e.getMessage());
		}	
	...
	public void alterar()throws ExcecaoDadoInvalido, ExcecaoElementoInexistente, ExcecaoRepositorioVazio {		
		...
		Administrador atd= this.fachada.buscarAdministrador(cpf01.getText());
		String nomeP = nome.getText();
		String cpfP = cpf01.getText();				
		String telefoneP = telefone.getText();		
		Administrador administrador = new Administrador(nomeP,cpfP,telefoneP,atd.getSenha());
		...
		if(opcao==0){
			this.fachada.alterar(administrador);
	...
	public void buscar() throws ExcecaoElementoInexistente, ExcecaoDadoInvalido{
		...
		Administrador administrador03 = fachada.buscarAdministrador(cpf01.getText());
		nome.setText(administrador03.getNome());
		cpf02.setText(administrador03.getCpf());				
		telefone.setText(administrador03.getTelefone());
	...
	public void remover() throws ExcecaoElementoInexistente, ExcecaoDadoInvalido{
		...
		int opcao= JOptionPane.showOptionDialog(null, "CONFIRMA A OPERAÇÃO?",null, JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
		if(opcao==0){
			this.fachada.removerAdministrador(cpf01.getText());
	...
	public void imprimir() throws ExcecaoRepositorioVazio, ExcecaoDadoInvalido, ExcecaoElementoInexistente{	
		textArea_2.append(this.fachada.imprimirAdministrador(cpf01.getText()));
	}	
	
	
	public void listar() throws ExcecaoRepositorioVazio{
		ArrayList<String> listaAdministradores = this.fachada.listarAdministradores();
		limparTextArea();
		for(int i=0;i<listaAdministradores.size();i++){
			textArea_2.append(listaAdministradores.get(i));
		}
	...
	public void trocarSenha() throws ExcecaoDadoInvalido,ExcecaoElementoInexistente{
		...
		Administrador administrador = this.fachada.buscarAdministrador(cpf01.getText());
		if(senhaAntiga.getText().equals(administrador.getSenha())){
			administrador.setSenha(novaSenha.getText());
			Object[] options = { "SIM", "NÃO" };
			int opcao= JOptionPane.showOptionDialog(null, "CONFIRMA A OPERAÇÃO?",null, JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
			if(opcao==0){
				try{
